...
We'll be needing some basic packages to help us along the way. Install [`forcats`](https://forcats.tidyverse.org/), [`readr`](https://readr.tidyverse.org/) and [`ggplot2`](https://ggplot2.tidyverse.org/).
...
Lets import the dataset of nuclei count. The dataset is found [here](https://github.com/KarlssonLaboratory/mi8016-Research-Approaches/blob/main/data/migration.csv). We will download it directly into R with the `read_csv()` function (`readr` packages)
...
Use the `forcats` function `fct_inorder()` to make the _name_ column in to factors (categorical) and in the order they appear.
...
The `plot()` function generates boxes for our two columns, `migration$Control` and `migration$PFOS`:
